...
class Leakcheck {
private:
    std::string api_base = "https://leakcheck.io/api";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("leakcheck.io"));
...
public:
    ...
    pplx::task<json::value> check_leak(const std::string& check){
        return make_api_call("/public?check=" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(check))),"GET");
    }
